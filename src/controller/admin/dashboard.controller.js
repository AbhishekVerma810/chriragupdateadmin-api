...
exports.adminDashboard = async (req, res, next) => {
  try {
    ...
    const TBA = await Customer_Visit.sum("transaction_amount", {
      where: { organization_id: adminOraganizationID },
    });
    console.log("TBA", TBA);
    ...
    const DA = await Customer_Visit.sum("received_loyalty_point", {
      where: { organization_id: adminOraganizationID },
    });
    ...
    console.log("Current Year Billing Amount:", currentYearBillingAmount);
    console.log("Last Year Billing Amount:", lastYearBillingAmount);

    // spending trends for last and thismonth
    const lastMonthSpendingTrend = totalBillingAmountOfLastMonth;
    ...
    const lastYearSpendingTrend = lastYearBillingAmount;
    const averageSpendTillCurrentDateOfThisYear = Math.round(
      currentYearBillingAmount / totalDaysTillCurrentDateCurrentYear
    );
    const thisYearSpendingTrend =
      averageSpendTillCurrentDateOfThisYear * totalDaysInYear;
...
exports.accessAdminDashboard = async (req, res, next) => {
  try {
    const error = req.flash("error");
    const message = req.flash("success");
    const formValue = req.flash("formValue")[0];
