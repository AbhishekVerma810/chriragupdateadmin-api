const { Delivery_Boy } = require("../../models");
...
exports.createDeliveryBoy = async (req, res, next) => {
  try {
    ...
    const existingUser = await Delivery_Boy.findOne({
    ...
    // Create a new user
    const newUser = await Delivery_Boy.create({
      name: req.body.name,
    ...
    await Delivery_Boy.update({ token: token }, { where: { id: newUser.id } });
    return Response.successResponseData(
      res,
      newUser,
      res.locals.__("Delivery Boy Create successfully")
    );
  } catch (err) {
    return Response.errorResponseWithoutData(
      res,
...
exports.login = async (req, res, next) => {
  try {
    ...
    const user = await Delivery_Boy.findOne({ where: { email:email } });
    if (!user) {
...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    const userToUpdate = await Delivery_Boy.findByPk(req.user.id);
...
exports.updatePassword = async (req, res, next) => {
  try {
    ...
    if (oldPassword && newPassword && confirmPassword) {
      const user = await Delivery_Boy.findByPk(1);
...
exports.getUserDetail = async (req, res, next) => {
  try {
    const user = await Delivery_Boy.findOne({
...
exports.getAllUserData = async (req, res, next) => {
  try {
    const user = await Delivery_Boy.findAll();
    if (!user) {
